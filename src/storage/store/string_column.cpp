...
void StringColumn::writeValue(ChunkState& state, offset_t offsetInChunk,
    ...
    updateStatistics(state.metadata, state.nodeGroupIdx, offsetInChunk, StorageValue(index),
        StorageValue(index));
}
...
void StringColumn::write(ChunkState& state, offset_t dstOffset, ColumnChunk* data,
    ...
    updateStatistics(state.metadata, state.nodeGroupIdx, dstOffset + numValues, minWritten,
        maxWritten);
}
