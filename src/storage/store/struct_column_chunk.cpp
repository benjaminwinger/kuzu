...
void StructColumnChunk::appendArray(
    ...
    case arrow::Type::STRUCT: {
        ...
        if (common::StructType::getNumFields(&dataType) != structArray->type()->fields().size()) {
            throw CopyException{
                "Unmatched number of struct fields in StructColumnChunk::appendVector."};
        }
        for (auto i = 0u; i < structArray->num_fields(); i++) {
            ...
            childrenChunks[fieldIdx]->appendArray(
    ...
    default: {
        throw NotImplementedException("StructColumnChunk::appendVector");
    }
    }
}

void StructColumnChunk::appendColumnChunk(ColumnChunk* other, offset_t startPosInOtherChunk,
    ...
    for (auto i = 0u; i < getNumChildren(); i++) {
        childrenChunks[i]->appendColumnChunk(otherStructChunk->childrenChunks[i].get(),
