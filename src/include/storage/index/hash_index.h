...
private:
    ...
    void copyAndUpdateSlotHeader(
        ...
        if constexpr (isCopyEntry) {
            memcpy((uint8_t*)&slot.entries[entryPos], &key,
                this->indexHeaderForWriteTrx->numBytesPerEntry);
        } else {
...
private:
    ...
    std::unique_ptr<HashIndexLocalStorage<Key, BufferKeyType>> localStorage;
