...
public:
    ...
    static KUZU_API LogicalType VAR_LIST(std::unique_ptr<LogicalType> childType);
    template<class T>
    static inline LogicalType VAR_LIST(T&& childType) {
        return LogicalType::VAR_LIST(std::make_unique<LogicalType>(std::forward<T>(childType)));
    }

    static KUZU_API LogicalType MAP(
        std::unique_ptr<LogicalType> keyType, std::unique_ptr<LogicalType> valueType);
    template<class T>
    static inline LogicalType MAP(T&& keyType, T&& valueType) {
