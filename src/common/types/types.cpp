...
std::unique_ptr<LogicalType> LogicalTypeUtils::parseVarListType(const std::string& trimmedStr) {
    return std::make_unique<LogicalType>(
        LogicalType::VAR_LIST(dataTypeFromString(trimmedStr.substr(0, trimmedStr.size() - 2))));
}
...
std::unique_ptr<LogicalType> LogicalTypeUtils::parseStructType(const std::string& trimmedStr) {
    return std::make_unique<LogicalType>(LogicalType::STRUCT(parseStructTypeInfo(trimmedStr)));
}
...
std::unique_ptr<LogicalType> LogicalTypeUtils::parseMapType(const std::string& trimmedStr) {
    ...
    return std::make_unique<LogicalType>(LogicalType::MAP(
...
std::unique_ptr<LogicalType> LogicalTypeUtils::parseUnionType(const std::string& trimmedStr) {
    ...
    return std::make_unique<LogicalType>(LogicalType::UNION(std::move(unionFields)));
}
...
LogicalType LogicalType::VAR_LIST(std::unique_ptr<LogicalType> childType) {
    return LogicalType(
...
LogicalType LogicalType::MAP(
    ...
    auto mapStructType =
        std::make_unique<LogicalType>(LogicalType::STRUCT(std::move(structFields)));
    return LogicalType(
