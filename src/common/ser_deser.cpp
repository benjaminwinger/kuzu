...
template<>
void SerDeser::serializeValue<std::string>(const std::string& value) {
    uint64_t valueLength = value.length();
    writeToBuffer((uint8_t*)&valueLength, sizeof(uint64_t));
    writeToBuffer((uint8_t*)value.data(), valueLength);
}
