...
OnDiskGraphVertexScanState::OnDiskGraphVertexScanState(ClientContext& context,
    ...
    : nodeIDVector{std::make_unique<ValueVector>(LogicalType::INTERNAL_ID(),
          context.getMemoryManager(), std::make_unique<DataChunkState>())},
      context{context},
    ...
    for (const auto& property : propertyNames) {
        auto propertyVector =
            std::make_unique<ValueVector>(tableCatalogEntry->getProperty(property).getType().copy(),
                context.getMemoryManager(), nodeIDVector->state);
